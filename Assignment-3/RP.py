...
def main():
    ...
    for i, dim in product(range(10), dim_red):
        rp = SparseRandomProjection(random_state=i, n_components=dim)
        tmp[dim][i] = pairwiseDistCorr(rp.fit_transform(madelon_X), madelon_X)
    tmp = pd.DataFrame(tmp).T
    tmp.to_csv(out + 'madelon scree1.csv')

    tmp = defaultdict(dict)
    for i, dim in product(range(10), dims_red_s):
        rp = SparseRandomProjection(random_state=i, n_components=dim)
        tmp[dim][i] = pairwiseDistCorr(rp.fit_transform(character_X), character_X)
    tmp = pd.DataFrame(tmp).T
    tmp.to_csv(out + 'character_scree1.csv')

    tmp = defaultdict(dict)
    for i, dim in product(range(10), dim_red):
        rp = SparseRandomProjection(random_state=i, n_components=dim)
        rp.fit(madelon_X)
        tmp[dim][i] = reconstructionError(rp, madelon_X)
    tmp = pd.DataFrame(tmp).T
    ...
    for i, dim in product(range(10), dims_red_s):
        rp = SparseRandomProjection(random_state=i, n_components=dim)
        rp.fit(character_X)
        tmp[dim][i] = reconstructionError(rp, character_X)
    tmp = pd.DataFrame(tmp).T
    tmp.to_csv(out + 'character_scree2.csv')

    # %% Data for 2

    grid = {'rp__n_components': dim_red, 'NN__alpha': nn_reg, 'NN__hidden_layer_sizes': nn_arch}
    ...
    grid = {'rp__n_components': dims_red_s, 'NN__alpha': nn_reg, 'NN__hidden_layer_sizes': nn_arch}
    rp = SparseRandomProjection(random_state=5)
    mlp = MLPClassifier(activation='relu', max_iter=2000, early_stopping=True, random_state=5)
    pipe = Pipeline([('rp', rp), ('NN', mlp)])
    gs = GridSearchCV(pipe, grid, verbose=10, cv=5)
    ...
    tmp = pd.DataFrame(gs.cv_results_)
    tmp.to_csv(out + 'character_dim red.csv')
    # raise
    # %% data for 3
    ...
    madelon_X2 = rp.fit_transform(madelon_X)
    ...
    dim = 16
    rp = SparseRandomProjection(n_components=dim, random_state=5)
    letter_X2 = rp.fit_transform(character_X)
    letter_2 = pd.DataFrame(np.hstack((letter_X2, np.atleast_2d(character_Y).T)))
    cols = list(range(letter_2.shape[1]))
    cols[-1] = 'Class'
    letter_2.columns = cols
    letter_2.to_hdf(out + 'datasets.hdf', 'character', complib='blosc', complevel=9)
